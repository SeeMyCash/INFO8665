#!/usr/bin/env python3
"""Train a bill denomination classifier locally (PyTorch).

Expected data layout (split-preserving ImageFolder):
...
def build_loaders(
    ...
    train_tfms = transforms.Compose(
        [
            ...
            transforms.ColorJitter(brightness=0.14, contrast=0.14, saturation=0.14, hue=0.03),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train bill denomination classifier")
    parser.add_argument("--train-dir", default=str(Path("data") / "processed" / "bill_cutouts" / "train"))
    parser.add_argument("--val-dir", default=str(Path("data") / "processed" / "bill_cutouts" / "val"))
    parser.add_argument("--output", default=str(Path("outputs") / "models" / "bill_classifier.pt"))
