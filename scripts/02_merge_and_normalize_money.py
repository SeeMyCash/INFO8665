#!/usr/bin/env python3
"""Merge and normalize banknotes + coins into one detector dataset.

This builds a unified YOLO dataset for a *money detector*.

Inputs:
  - data/interim/banknotes/**/data.yaml
  - data/interim/coins/**/data.yaml
Config:
  - configs/label_map_money.yaml
Output:
  - data/processed/money_merged/

Coin datasets are normalized to a single canonical class: COIN.
"""
...
def build_remap(src_names: dict[int, str], raw_map: dict[str, str], canonical: list[str]) -> dict[int, int]:
    ...
    for src_idx, src_name in src_names.items():
        ...
        if "coin" in norm or norm in {"penny", "nickel", "dime", "quarter", "loonie", "toonie"}:
            if "coin" in canonical_lower:
                remap[src_idx] = canonical_lower["coin"]
                continue
            if "COIN" in canonical:
                remap[src_idx] = canonical.index("COIN")
                continue
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Merge and normalize banknotes + coins")
    parser.add_argument("--banknotes-root", default=str(Path("data") / "interim" / "banknotes"))
    parser.add_argument("--coins-root", default=str(Path("data") / "interim" / "coins"))
    parser.add_argument("--label-map", default=str(Path("configs") / "label_map_money.yaml"))
    parser.add_argument("--out", default=str(Path("data") / "processed" / "money_merged"))
    ...
    data_yamls = find_data_yamls(Path(args.banknotes_root)) + find_data_yamls(Path(args.coins_root))
    ...
    (out_root / "data.yaml").write_text(yaml.dump(content, default_flow_style=False), encoding="utf-8")
