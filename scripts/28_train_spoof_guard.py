#!/usr/bin/env python3
"""Train a spoof-guard classifier (real vs spoof) for pre-detector protection.
...
def build_loaders(
    ...
    train_tfm_steps = [
        ...
        transforms.ColorJitter(brightness=0.20, contrast=0.20, saturation=0.16, hue=0.04),
    ...
    if normalize_imagenet:
        train_tfm_steps.append(transforms.Normalize(IMAGENET_MEAN, IMAGENET_STD))
        eval_tfm_steps.append(transforms.Normalize(IMAGENET_MEAN, IMAGENET_STD))

    train_ds = datasets.ImageFolder(str(train_dir), transform=transforms.Compose(train_tfm_steps))
    val_ds = datasets.ImageFolder(
        str(val_dir if val_dir is not None else train_dir),
        transform=transforms.Compose(eval_tfm_steps),
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train spoof guard classifier")
    parser.add_argument(
        "--train-dir",
        default=str(Path("data") / "processed" / "spoof_guard" / "train"),
        help="ImageFolder train root",
    )
    parser.add_argument(
        "--val-dir",
        default=str(Path("data") / "processed" / "spoof_guard" / "val"),
        help="ImageFolder val root",
    )
    parser.add_argument("--output", default=str(Path("outputs") / "models" / "spoof-guard-local__model.pt"))
    ...
    parser.add_argument("--normalize-imagenet", type=str2bool, default="true")
    ...
    normalize_imagenet = bool(str2bool(str(args.normalize_imagenet)))
